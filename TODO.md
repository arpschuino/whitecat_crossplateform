...
- [ ] **À CREUSER (demain) — cause amont du garbage dans `grid_goto`** : le clamp ci-dessus empêche le crash, mais le log a montré `[grid preset clamp] pl=0 grid=0 step=0 preset0=559903 preset1=0 goto_mode=1 seq=0` → `grid_goto[0][0][0] = 559903` (index de grille garbage) alors que le mode goto était actif. Le clamp masque le crash mais le goto résout vers la grille 127 (faux). Deux axes :
  1. **Handling propre du goto** : au test `grid_goto[…][0]>=0 && grid_goto[…][1]>=0` (grider_calcul.cpp:269-270), ajouter la **borne haute** (`<=127` / `<=1023`) → un goto corrompu est *ignoré* (progression normale) au lieu d'être clampé vers la grille 127.
  2. **Origine du 559903** : init manquante de `grid_goto` à -1 au démarrage/reset (BSS=0, pas -1 ; est-ce lu comme goto valide ?) ? vieux show mal relu (alignement/format `file_grid_goto`) ? écriture hors bornes ailleurs qui corrompt `grid_goto` ? → à reproduire : le `[grid preset clamp]` apparaît-il avec un **show neuf** (grille jamais chargée) ou seulement sur ce show précis ? (Objectif Jacques : garder le soft **sain**, pas seulement non-crashant.)
