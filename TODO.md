...
- [ ] **Devices : attributs non pilotés (paramètres de projecteur inutilisés)** — l'import GDTF ne mappe qu'un sous-ensemble (`Dimmer/Pan/Tilt/RGBW/Zoom/Shutter1`) ; tous les autres canaux (gobo, prisme, roue de couleur, CTC, focus, iris, control/function, macros…) deviennent `ATTR_RAW` (réservés au footprint, sortie 0, **non pilotables** dans Control Fixtures). À terme : étendre `wc::AttrId` (channel.h) + le mapping (`attr_from_gdtf` dans gdtf_import.cpp) + les rouleaux (fxc_* fixturectl_visu.cpp), et gérer les canaux à **valeur par défaut** (ex. Control « Idle », Shutter « open »). Signalé par Jacques (12/07).
...
- **Stockage des scènes en 16 bit — vers le 16 bit TOTAL** (⚠️ FONDAMENTAL têtes mobiles). Objectif validé avec Jacques : **16 bit complet et total** à terme. Approche par étapes (échafaudage 8 bit tag `[mem16 s1]` à retirer au fil des stages) :
  ...
  - [x] **Stage C — Wizard (maths)** — FAIT (0.10-devices). Set/Add/Reduce en 16 bit natif : Set % via `pct_to_lvl` (plus de `×2.55`) ; Add/Reduce sans round-trip 8 bit du niveau courant (finesse conservee), clamps 0-65535 ; vue DMX via `dmx8_to_lvl`. Wrappers `[mem16 s1]` retires de wizard_operations.cpp. Validé.
