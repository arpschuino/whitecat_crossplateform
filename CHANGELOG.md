...
- **Noms éditables au double-clic** (fenêtre Save, modes *Binary* et *Import/Export*) : double-cliquer sur un champ de nom permet de le saisir **directement**, sans passer par la fenêtre texte F5 (curseur blanc, flèches, backspace, UTF-8) ; Entrée valide, Échap annule, un clic sur le bouton d'action valide aussi le nom en cours (et re-détecte le format à l'import). Repose sur un **composant d'édition inline réutilisable** (`wc_inline_edit`) : ajouter cette saisie directe à un nouveau champ tient en trois lignes, sans code dupliqué — préparé pour être étendu à d'autres endroits de l'interface.
