...
- **Fix : ouvrir la fenêtre Faders (F10 ou clic) faisait quitter WhiteCat instantanément** avec certains shows (signalé par un beta sous Linux). Cause : `string_docktypfx` était un `char[8]` alors que le rendu y écrit l'étiquette de type de dock — `"Grid P.%d"` fait déjà **9 octets** avec le `\0` (« Grid P.1 » = 8 caractères), donc **débordement de tampon systématique** dès qu'un dock est de type Grid, Draw ou Echo (et bien pire avec un index résiduel : « Grid P.1000 » = 12 octets). Buffer porté à 16. Bénin sous Windows, **fatal sous Linux** (le débordement d'un buffer global y tombe sur une zone non mappée) — d'où le fait qu'il ne se reproduisait pas côté Windows. *(Les minifaders n'étaient pas touchés : ils ne dessinent pas ces étiquettes.)*
- **Durcissement de la même zone** : gardes de bornes ajoutées sur les index de dock avant tout accès tableau — la sentinelle « non assigné » (`ChaserAffectedToDck` = 999 par défaut) était utilisée telle quelle comme index. Bornage chaser `[0,128[`, gridplayer `[0,4[`, players audio `[0,4[` (rendu du fader, copies de nom `chaser_name`/`grider_name`, boutons play/loop audio embarqués). Et `detect_dock_used()` ne renvoie plus une variable non initialisée quand aucun dock sélectionné n'est atteint (fallback dock 0).

### LFO / chasers — saccade au démarrage d'une animation (Linux)

- **Fix : la progression d'un fader (saw/LFO) et le clic sur Loop étaient saccadés au début du mouvement**, y compris sur une tranche non concernée (signalé par un beta sous Linux ; « moindre avec plusieurs cœurs »). L'automation (LFO, chasers, crossfade) tourne sur le thread du ticker ; le rendu dort dans `SDL_WaitEventTimeout` sur le thread principal. `wc_request_refresh()` posait un drapeau mais ne **réveillait** pas le thread principal → la première frame pouvait être retardée jusqu'à ~1 s en mode idle. `wc_request_refresh()` pousse désormais un `SDL_USEREVENT` throttlé (~26/s, aligné sur le cap 25 fps) qui réveille immédiatement la boucle de rendu. La sortie DMX était déjà correcte (bug purement à l'écran).

### Robustesse — chemins et chargement

- **`chdir` vers le dossier de l'exécutable à la sortie** : garantit que les assets (`Fonts/`, `gfx/`, `user/`) restent trouvés à côté de l'exe (utile sous Linux / lancement depuis un autre répertoire de travail).
- **Fix : lecture sur un `FILE*` invalide dans `load_show_coming_from()`** — une accolade mal placée faisait exécuter `fgets`/`fclose` **hors** de la branche « fichier ouvert avec succès » quand `show_coming_from.txt` était absent. Bloc correctement refermé.
...
- **Help : repli sur la doc en ligne si `doc/` est absent** (paquet Linux sans le dossier `doc/` à côté de l'exe) → ouverture de la version en ligne (`arpschuino.github.io`) au lieu de ne rien faire. Le bouton reste allumé ~1,2 s puis se réinitialise (retour visuel que le clic a été pris).
- **Fix : zone de clic du numéro d'univers Art-Net décalée** (CFG > network, champ « univers à écouter », à côté de *Affect to dock*). La zone cliquable était en `y[+25,+47]` alors que la case est dessinée en `y[+12,+37]` → il fallait cliquer dans le bas de la case ou juste en dessous. Zone alignée sur la case.
- **Fix : les choix « do calculation » (CFG core) n'étaient pas retrouvés à l'ouverture.** Ces interrupteurs (familles de calcul actives : LFO, crossfade, chasers, grids, echo…) sont une préférence globale sauvée dans `config_core.txt` (au clic *SaveCFG*) et rechargée au démarrage — mais `GlobInit()` les remettait tous à *on*, et `GlobInit` est appelé par `Load_Show()` qui tourne **après** le rechargement → les choix étaient écrasés (les compteurs nb_faders/nb_docks, eux, survivaient car `GlobInit` n'y touche pas). Corrigé comme les compteurs : défaut à *on* à froid + `GlobInit` ne les réinitialise plus. Sauvegarde inchangée (uniquement au clic *SaveCFG*). Signalé par un beta.
