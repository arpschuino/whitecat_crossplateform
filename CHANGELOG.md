...
- **Export cohérent Eos-flavored.** Tout l'export est désormais dans le même dialecte (avant : cues en TitleCase mais patch/subs en MAJUSCULES) : `Patch 1 3<3@Hff`, `Sub 1 1` / `Up` / `Down` / `Chan 14@Hff`. Les **noms de dock (submasters) sont exportés** (ils manquaient) ; les docks vides et sans nom ne sont plus écrits.
- **Noms accentués à l'export en Latin-1.** Les labels (`Text …`) sont écrits en Latin-1 / CP1252 — ce qu'attendent Eos et Cobalt — au lieu de l'UTF-8 interne (qui sortait « Mémoire » en « MÃ©moire » sur ces consoles). Le round-trip WhiteCat reste correct (l'import reconvertit Latin-1 → UTF-8).
- **Master-mémoire exporté (`$$SubMem`).** Un fader qui rappelle une mémoire (dock type 5) est exporté avec la mémoire **aplatie en niveaux** (utilisable telle quelle sur Eos/Cobalt) **plus** une ligne `$$SubMem <n>`. À la relecture, WhiteCat restaure le vrai master-mémoire (lien dynamique vers la mémoire) ; les autres consoles ignorent la ligne `$$` et gardent l'instantané.
- **Export Congo/Cobalt (toggle « Format: Eos / Congo »).** Un vrai export Cobalt échoue en partant de zéro : son fichier natif contient des dizaines de sections obligatoires (Parameter Definitions, Channel Database ~5000 lignes, Devices…) — un fichier incomplet **fait planter** la console. On procède donc par **« template-splice »** : un vrai squelette Cobalt (`import_export/congo_template.asc`) est recopié à l'identique et WhiteCat n'y **injecte que ses données** — cues (bloc `CUE`/`$$CUEPARAMS`/`$$GOONGO`/`CHAN x/HXX`/`$$PRESETTEXT` Latin-1 + `$$TEXT` entités HTML), patch (`PATCH 1 x<y@100`) et masters (`$MASTPAGEITEM` sous chaque `$MASTPAGE`). Le fichier s'ouvre dans Cobalt (File → Open) avec cues + patch + masters, **temps et délais de transfert** compris (`DOWN <temps> <délai>` / `UP <temps> <délai>`, valeurs assainies contre tout `%g` invalide qui ferait planter la console). Le toggle **Format: Eos / Congo** dans la fenêtre Import/Export choisit le dialecte.
- **Submasters Eos → docks de faders.** Les submasters Eos (`Sub 1 1` + `Text`/`Up`/`Down`/`Chan`) sont désormais importés : tout le bloc est lu insensiblement à la casse (avant, seul le `SUB` majuscule de WhiteCat passait) et les niveaux sont parsés en `@H` (Eos) comme en `/H` (WhiteCat). Mapping `sub N → fader N-1, dock N/48` ; le nom du sub va dans le label du dock (UTF-8), et le dock cible est remis à zéro avant remplissage. *(Complète l'import des masters Congo par `$$MASTPAGEITEM`.)*
