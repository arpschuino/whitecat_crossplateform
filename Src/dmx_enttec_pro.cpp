...
#define READ_ONE_BYTE(handle, byte, bytes_read, overlapped_struct)                                                     \
    ReadFile(handle, byte, 1, bytes_read, overlapped_struct)
...
int Enttec_Pro_SendData(int label, unsigned char *data, unsigned int length, LPOVERLAPPED lpOverlapped) {
    ...
    res = WriteFile(com_handle_,             // Instance de votre accès au port série
...
int Detect_EnttecProOut() {
    ...
    // Search in Windows Registry for serial COM
    {
        ...
        char KeyName[256];
        int idmx;
        if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, "HARDWARE\\DEVICEMAP\\SERIALCOMM", 0, KEY_QUERY_VALUE, &hKey) !=
            ERROR_SUCCESS) {
        ...
        while (idmx < 50) {
            DeviceNameLen = 80;
            KeyNameLen = 100;
            if (RegEnumValue(hKey, idmx, KeyName, &KeyNameLen, NULL, NULL, DeviceName, &DeviceNameLen) != ERROR_SUCCESS)
                break;

            if (((!strncmp(KeyName, "\\Device\\VCP", 11)) || (!strncmp(KeyName, "\\Device\\USBSER", 14))) &&
                idmx != arduino_com0) {
                index_init_dmx_ok = 1;
...
int Open_EnttecProOut() {
    char com_str[32];
    sprintf(com_str, "\\\\.\\%s", DeviceName); // format universel, fonctionne pour COM1-COM99+
    LPSTR lpszPortName = _T(com_str);
    com_handle_ = CreateFile(lpszPortName, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);

    if (com_handle_ == INVALID_HANDLE_VALUE) {
        com_handle_ = NULL;
        index_init_dmx_ok = 0;
        sprintf(string_display_dmx_params, "Impossible to open %s, is it PLUGGED ?", DeviceName);
        return (0);
    }

    Enttec_Pro_SetCommParams();

    // flush - certains périphériques USB-série ne le supportent pas, on ignore l'erreur
    FlushFileBuffers(com_handle_);

    sprintf(string_display_dmx_params, "ENTTEC PRO Out opened on %s", DeviceName);
    return (0);
}
...
int Detect_EnttecProIn() {
    ...
    // Search in Windows Registry for serial COM
    {
        ...
        char KeyNameIN[256];
        int idmxIN;
        if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, "HARDWARE\\DEVICEMAP\\SERIALCOMM", 0, KEY_QUERY_VALUE, &hKeyIN) !=
            ERROR_SUCCESS) {
        ...
        while (idmxIN < 50) {
            DeviceNameLenIN = 80;
            KeyNameLenIN = 100;
            if (RegEnumValue(hKeyIN, idmxIN, KeyNameIN, &KeyNameLenIN, NULL, NULL, DeviceNameIN, &DeviceNameLenIN) !=
                ERROR_SUCCESS)
                break;
...
int Open_ProIn() {
    ...
    LPSTR lpszPortName = _T(com_str);
    com_handle_IN = CreateFile(lpszPortName, GENERIC_READ | GENERIC_WRITE,
                               0,             // DWORD dwShareMode,
