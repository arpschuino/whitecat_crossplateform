...
int Get_channels_from_memory(int the_mem)
{
    for(int p=1; p<513; p++)
    {
        ...
        {
            if (!index_blind) {
                bufferSaisie[p]=Memoires[the_mem][p];   // [mem16] rappel memoire 16 bit direct
            } else {
                bufferBlind[p]=Memoires[the_mem][p];   // [mem16]
            }
        }
    }
...
int patch_straight()
{
    ...
    // [devices] patch droit = remplacement total "tout dimmer" : on lache tous les devices,
...
int set_channel_scroll( int ch)
{
    ...
    {
        // [fix CH.view] offset -3 (= valeur validée au startup, MAIN.cpp:1216) : sans lui la 1re
        // rangée de chaque page de 48 est dessinée en Y=36 (YChannels=70, Ch_Scroll_Factor=12),
        // donc cachée sous la barre Ch.View (clip à ~53). -3 descend la rangée à Y=72.
        if(ch>0 && ch<48)
        {
...
int record_memory(int mem_is)
{
    for (int u=1; u<514; u++)
    {
        if(index_blind==0)
        {
            Memoires[mem_is][u]=bufferSequenciel[u];   // [mem16] enregistrement 16 bit direct
        }
        else if(index_blind==1)
        {
            Memoires[mem_is][u]=bufferBlind[u];   // [mem16]
        }
...
int record_memory_plus_faders(int mem_is)
{
    if(index_blind==0)
    {
        ...
        for (int u=1; u<514; u++)
        {
            ...
            {
                Memoires[mem_is][u]=bufferSequenciel[u];   // [mem16] 16 bit direct
            }
            if(bufferSequenciel[u] <=bufferFaders[u])
            {
                Memoires[mem_is][u]=bufferFaders[u];   // [mem16] 16 bit direct
            }
            Selected_Channel[u]=0;
        }
