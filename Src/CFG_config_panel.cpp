...
int do_keyboard_conf(int cfgnetw_X, int cfgnetw_Y) // ancienne version
{
    ...
    for (int oi = 0; oi < nbre_key_persos; oi++) {
        ...
        if (mouse_x > cfgnetw_X + 60 && mouse_x < cfgnetw_X + 60 + 50 && mouse_y > cfgnetw_Y + 25 + (oi * 30) &&
            mouse_y < cfgnetw_Y + 25 + (oi * 30) + 25) {
            keyParam.DrawOutline(CouleurLevel);
            if (mouse_button == 1 && mouse_released == 0) {
                int tmp_valAsc = atoi(numeric);
                if ((tmp_valAsc > 0 && tmp_valAsc < 255) ||
                    tmp_valAsc ==
                        999) // 999 pour eviter de bloquer les commandes en 0 ascii , genre touches de fonctions
                {
...
int do_core_config(int x_cfg_sc, int y_cfg_sc, int largeur_cfg_sc, int hauteur_cfg_sc) {
    ...
    for (int l = 0; l < 7; l++) {
        ...
        case 3:
            ...
            petitchiffre.Print(ol::ToString(core_user_define_nb_tracks_per_chasers), x_cfg_sc + 135,
                               y_cfg_sc + 75 + (l * 30));
            def_param_max = 24;
            break;
        case 4:
        ...
        if (window_focus_id == 920 && mouse_x > x_cfg_sc + 120 && mouse_x < x_cfg_sc + 120 + 50 &&
            mouse_y > y_cfg_sc + 60 + (l * 30) && mouse_y < y_cfg_sc + 60 + (l * 30) + 25) {
    ...
    for (int l = 0; l < 2; l++) {
        ...
        if (window_focus_id == 920 && mouse_x > x_cfg_sc + 470 && mouse_x < x_cfg_sc + 470 + 50 &&
            mouse_y > y_cfg_sc + 60 + (l * 30) && mouse_y < y_cfg_sc + 60 + (l * 30) + 25) {
...
int do_main_config(int cfgnetw_X, int cfgnetw_Y, int largeurCFGdmxwindow, int hauteurCFGdmxwindow) {
    ...
    for (int oi = 0; oi < 5; oi++) {
        ...
        if (window_focus_id == W_CFGMENU && mouse_x > cfgnetw_X + 155 && mouse_x < cfgnetw_X + 155 + 50 &&
            mouse_y > cfgnetw_Y + 55 + (oi * 30) && mouse_y < cfgnetw_Y + 55 + (oi * 30) + 25) {
    ...
    for (int oi = 0; oi < 8; oi++) {
        ...
        UnderParam.SetLineWidth(epaisseur_ligne_fader);
        UnderParam.Draw(CouleurFond.WithAlpha(0.5));
        if (window_focus_id == W_CFGMENU && mouse_x > cfgnetw_X + 365 && mouse_x < cfgnetw_X + 365 + 50 &&
            mouse_y > cfgnetw_Y + 55 + (oi * 30) && mouse_y < cfgnetw_Y + 55 + (oi * 30) + 25) {
            UnderParam.DrawOutline(CouleurLevel);
            if (mouse_button == 1 && mouse_released == 0) {
        ...
        case 2:
            ...
            petitchiffre.Print(ol::ToString(index_affichage_digit_time_unit + 1), cfgnetw_X + 380,
                               cfgnetw_Y + 70 + (oi * 30));
            break;
        case 3:
        ...
        case 5:
            ...
            petitchiffre.Print(ol::ToString(nbre_memoires_visualisables_en_preset), cfgnetw_X + 380,
                               cfgnetw_Y + 70 + (oi * 30));
            break;
        case 6:
    ...
    if (window_focus_id == W_CFGMENU && mouse_x > cfgnetw_X + 525 && mouse_x < cfgnetw_X + 525 + 50 &&
        mouse_y > cfgnetw_Y + 55 && mouse_y < cfgnetw_Y + 55 + 20) {
    ...
    if (window_focus_id == W_CFGMENU && mouse_x > cfgnetw_X + 525 && mouse_x < cfgnetw_X + 525 + 50 &&
        mouse_y > cfgnetw_Y + 85 && mouse_y < cfgnetw_Y + 85 + 20) {
    ...
    if (window_focus_id == W_CFGMENU && mouse_x > cfgnetw_X + 525 && mouse_x < cfgnetw_X + 525 + 50 &&
        mouse_y > cfgnetw_Y + 190 && mouse_y < cfgnetw_Y + 190 + 20) {
    ...
    if (window_focus_id == W_CFGMENU && mouse_x > cfgnetw_X + 525 && mouse_x < cfgnetw_X + 525 + 50 &&
        mouse_y > cfgnetw_Y + 215 && mouse_y < cfgnetw_Y + 215 + 20) {
    ...
    if (window_focus_id == W_CFGMENU && mouse_x > cfgnetw_X + 525 && mouse_x < cfgnetw_X + 525 + 50 &&
        mouse_y > cfgnetw_Y + 265 && mouse_y < cfgnetw_Y + 265 + 20) {
    ...
    if (window_focus_id == W_CFGMENU && mouse_x > cfgnetw_X + 740 && mouse_x < cfgnetw_X + 740 + 50 &&
        mouse_y > cfgnetw_Y + 225 && mouse_y < cfgnetw_Y + 225 + 20) {
...
int SelectDmxDevice(int interfacedmx) {
    if (dmx_interface_active[interfacedmx]) {
        dmx_interface_active[interfacedmx] = 0;
        Close_single_dmx_interface(interfacedmx);
        if (interfacedmx != 1)
            index_art_polling = 0;
    } else {
        dmx_interface_active[interfacedmx] = 1;
        myDMXinterfaceis = interfacedmx;
        if (interfacedmx != 1)
            index_art_polling = 0;
        Init_single_dmx_interface(interfacedmx);
    }
    index_init_dmx_ok = 0;
    for (int i = 1; i <= 4; i++) {
        if (dmx_interface_active[i]) {
            index_init_dmx_ok = 1;
            break;
        }
    }
...
int affect_dmx_in(int callb_x, int callb_y) {
    ...
    if (window_focus_id == W_CFGMENU && mouse_x > callb_x && mouse_x < callb_x + 100 && mouse_y > callb_y &&
        mouse_y < callb_y + 30 && window_focus_id == 920) {
        if (mouse_button == 1 && mouse_released == 0) {
...
int do_dmx_config(int cfgdmx_X, int cfgdmx_Y, int largeurCFGdmxwindow, int hauteurCFGdmxwindow) {
...

    {
        ...
        if (window_focus_id == W_CFGMENU && mouse_x > cfgdmx_X + 230 && mouse_x < cfgdmx_X + 300) {
            if (mouse_y > (cfgdmx_Y + 60) && mouse_y < (cfgdmx_Y + 80) && dmx_interface_active[1]) // artnet
            {
        ...
        if (window_focus_id == W_CFGMENU && mouse_x > cfgdmx_X + 320 && mouse_x < cfgdmx_X + 490) {
            if (mouse_y > (cfgdmx_Y + 60) && mouse_y < (cfgdmx_Y + 80) && dmx_interface_active[1]) // artnet
            {
                ...
                if (mouse_button == 1 && mouse_released == 0 && numeric_postext > 0) {
                    Close_single_dmx_interface(1);
                    sprintf(ip_artnet, numeric);
                    reset_numeric_entry();
                    Init_single_dmx_interface(1);
                    mouse_released = 1;
                }
            }
        }
        ...
        if (window_focus_id == W_CFGMENU && mouse_x > cfgdmx_X + 520 && mouse_x < cfgdmx_X + 620) {
            if (mouse_y > (cfgdmx_Y + 60) && mouse_y < (cfgdmx_Y + 80) &&
                dmx_interface_active[1]) // artnet BROADCAST MODE   ON OFF
            {
        ...
        if (window_focus_id == W_CFGMENU && mouse_x > cfgdmx_X + 670 && mouse_x < cfgdmx_X + 770) {
            if (mouse_y > (cfgdmx_Y + 60) && mouse_y < (cfgdmx_Y + 80)) // artpoll for devices
            {
                ...
                if (mouse_button == 1 && mouse_released == 0) {
                    if (!dmx_interface_active[1]) // dans le cas d un art poll avec interface usb
                    {
                    ...
                    if (index_art_polling == 0) {
                        ...
                        nbrbytessended =
                            sendto(sockartnet, ArtPollBuffer, sizeof(ArtPollBuffer), 0, (SOCKADDR *)&sinS, sinsize);
        ...
        if (window_focus_id == W_CFGMENU && mouse_x > cfgdmx_X + 270 && mouse_x < cfgdmx_X + 420 &&
            mouse_y > cfgdmx_Y + 120 && mouse_y < cfgdmx_Y + 140) {
        ...
        // sunlite allows IN
        if (window_focus_id == W_CFGMENU && mouse_x > cfgdmx_X + 240 && mouse_x < cfgdmx_X + 390 &&
            mouse_y > cfgdmx_Y + 180 && mouse_y < cfgdmx_Y + 210 && dmx_interface_active[4]) {
    ...
    /////////////////////////////////////////////////////////////////////////////////////
    if (dmx_interface_active[1]) {
        DeviceArtNetDevice.DrawOutline(CouleurLevel);
    }
    if (dmx_interface_active[2]) {
...
int config_general_menu() {
    ...
    for (int choix = 0; choix < 7; choix++) {
        ...
        case 0:
            ...
            if (window_focus_id == W_CFGMENU && mouse_x > mysetupx + (choix * 100) &&
                mouse_x < mysetupx + (choix * 100) + 90 && mouse_y > window_cfgY + 5 &&
                mouse_y < window_cfgY + 5 + 40) {
                if (mouse_button == 1 && mouse_released == 0 && window_focus_id == 920) {
                    reset_indexes_conf();
                    index_config_dmx = 1;
                    config_page_is = 1;
