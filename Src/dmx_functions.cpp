...
*   Fonction global d'écriture du dmx vers toutes les interfaces et l'arnet, gère aussi l'écriture direct du signal dmx
par le processeur du pc, envoyer vers l'enttec open via les drivers D2XX
*
*   Global fonctions to write the dmx to the diferente interface and artnet, write too directly the dmx signal from the
PC CPU to be send to the enttec open dmx via the D2XX drivers
*
 **/
...
int do_crossfade() {
    ...
    if (index_go == 1 && index_pause == 0) {
        ...
        if (niveauX1 == 0 && niveauX2 == 255) {
            ...
            if (auto_reset_crossfade_speed_on_link == 1 &&
                (Links_Memoires[position_onstage] == 0 || index_link_is_on == 0)) {
                crossfade_speed = 64;
            }
                ...
                if (show_gridplayer_in_seq == 1) {
                    ...
                    if (set_from_seq_gridplayer1_next_step[position_preset] != -1 &&
                        ...
                        gridder_prepare_cross(0, index_grider_selected[0],
                                              set_from_seq_gridplayer1_next_step[position_preset] - 1);
    ...
    if (lfo_speed[cmptfader] < 64) {
        ...
        time_delay_in[cmptfader] =
            (int)(time_per_dock[cmptfader][dksel][0] *
                  (((float)BPS_RATE * 3) /
                   (lfo_speed[cmptfader] - 62))); //-62 evite un passage de temps pas bon, cf curseur
        time_delay_out[cmptfader] =
            (int)(time_per_dock[cmptfader][dksel][2] * (((float)BPS_RATE * 3) / (lfo_speed[cmptfader] - 62)));
    }
...
int do_lfos() {
    for (int cmptfader = 0; cmptfader < core_user_define_nb_faders; cmptfader++) {
        ...
        if (is_dock_for_lfo_selected[cmptfader][dockis] == 0 && lfo_cycle_is_on[cmptfader] == 0) // ONCE CASE
        {
            ...
            // down only once
            else if (lfo_mode_is[cmptfader] == 2 &&
                     actual_time > (start_time_for_delays[cmptfader] + time_delay_out[cmptfader])) {
                index_lfoing[cmptfader] = 1;
        ...
        if (is_dock_for_lfo_selected[cmptfader][dockis] == 1 &&
            lfo_cycle_is_on[cmptfader] == 0) // LOOP LFO ON ONE DOCK ONLY ( NO DOCK CYCLING)
        {
            // up On Fade IN only
            if (lfo_mode_is[cmptfader] == 1 &&
                actual_time > (start_time_for_delays[cmptfader] + time_delay_in[cmptfader])) {
                ...
                if (ActionnateStopOn[cmptfader] == 1 && StopPosOn[cmptfader] == 1 &&
                    (Fader[cmptfader] == LevelStopPos[cmptfader] || Fader[cmptfader] > LevelStopPos[cmptfader]) &&
                    FaderIsFlash[cmptfader] == 0) { // Fader[cmptfader]=LevelStopPos[cmptfader];
                    fader_set_level(cmptfader, LevelStopPos[cmptfader]);
                }
                // report midi
                midi_levels[cmptfader] = (Fader[cmptfader] / 2);
        ...
        if (lfo_cycle_is_on[cmptfader] == 1) // LOOP LFO ( CYCLING + TIMES)
        {
            ...
            // down
            else if (lfo_running_is_upward[cmptfader] == 0 &&
                     actual_time > (start_time_for_delays[cmptfader] + time_delay_out[cmptfader])) {
                faders_in_float[cmptfader] -= fraction_lfo_out[cmptfader];
                if (faders_in_float[cmptfader] < 0) {
                    faders_in_float[cmptfader] = 0;
        ...
        if (index_lfoing[cmptfader] == 1) {
            ...
            if (FaderLocked[cmptfader] == 1 &&
                LockFader_is_FullLevel[cmptfader] == 1) // masterisation au lock des autres faders
            {
        ...
        } else if (lfo_cycle_steps[cmptfader] == 1 && lfo_do_next_step[cmptfader][1] == 1 &&
                   dockis == core_user_define_nb_docks - 1) {
                    ...
                    for (int ppin = 1; ppin < 513; ppin++) {
                        if (ppin < grider_begin_channel_is - 1) {
                            FaderDockContains[f][d][ppin] =
                                0; // nettoyage à cause de l adressage   si chgt adresse faut nettoyer le buffer
                        }
                    ...
                    for (int ppin = 1; ppin < 513; ppin++) {
                        ...
                        for (int p = 0; p < 500; p++) {
                            if (draw_preset_channel_routing[(DrawAffectedToDck[f][d])][p] == ppin) {
                                FaderDockContains[f][d][ppin] =
                                    int(255 * draw_preset_levels[(DrawAffectedToDck[f][d])][p]);
                            }
                        }
                    }
                    //  }
...
int load_dmx_conf() {
    ...
    fscanf(cfg_file, "%s\n", motcleinterfaceis);
    fscanf(cfg_file, "%d / %d / %d / %d /\n", &dmx_interface_active[1], &dmx_interface_active[2],
           &dmx_interface_active[3], &dmx_interface_active[4]);

    fclose(cfg_file);

    // backward compat: if keyword line parsed (old format), use it
    if (dmx_interface_active[1] == 0 && dmx_interface_active[2] == 0 && dmx_interface_active[3] == 0 &&
        dmx_interface_active[4] == 0) {
        if (strcmp(motcleinterfaceis, "ART-NET") == 0) {
            dmx_interface_active[1] = 1;
        } else if (strcmp(motcleinterfaceis, "ENTTEC-OPEN-DMX") == 0) {
            dmx_interface_active[2] = 1;
        } else if (strcmp(motcleinterfaceis, "ENTTEC-PRO") == 0) {
            dmx_interface_active[3] = 1;
        } else if (strcmp(motcleinterfaceis, "SUNLITE") == 0) {
            dmx_interface_active[4] = 1;
        }
    }
    myDMXinterfaceis = 0;
    for (int i = 1; i <= 4; i++) {
...
int Save_my_dmx_conf() {
    ...
    if ((fp = fopen(rep_conf_dmx, "w"))) {
        ...
        fprintf(fp, "%s\n", motcleinterface);
        fprintf(fp, "%d / %d / %d / %d /\n", dmx_interface_active[1], dmx_interface_active[2], dmx_interface_active[3],
                dmx_interface_active[4]);
    }
    fclose(fp);
    if (dmx_interface_active[1]) {
        save_artnet_conf();
...
int Receive_DMX_IN() {
    if (dmx_interface_active[4]) // sunlite
    {
        Receive_sunlite_dmxIN();
    }

    else if (index_init_EnttecPROIN_ok == 1 && !dmx_interface_active[4]) {
        // memset(dmxIN,0,513);
