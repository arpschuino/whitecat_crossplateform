...
else if( index_do_dock==1 && index_affect_time==0 )
{
 ...
 if(index_blind==0)
 {
 Memoires[mem_is][dc]=bufferSequenciel[dc];   // [mem16] 16 bit direct
 }
 else if(index_blind==1)
 {
 Memoires[mem_is][dc]=bufferBlind[dc];   // [mem16] 16 bit direct
 }
...
if(index_blind==0)
 ...
 for (int dc=1;dc<514;dc++)
 {
 if(bufferFaders[dc]>bufferSequenciel[dc]){Memoires[mem_is][dc]=bufferFaders[dc];}   // [mem16] 16 bit direct
 else { Memoires[mem_is][dc]=bufferSequenciel[dc]; }   // [mem16] 16 bit direct
 Selected_Channel[dc]=0;
 }
 for (int u=0;u<49;u++)
 {Fader[u]=0;}
